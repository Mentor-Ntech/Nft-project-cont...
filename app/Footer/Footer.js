...
const FooterLinks = ({ heading, items }) => {
  <div className='flex-1 justify-start items-start'>
    ...
    {items.map((item, index) => (
      <p key={index} className='font-poppins dark:text-while text-nft-black-1 font-normal text-base cursor-pointer dark:hover:text-nft-gray-1 hover:text-nft-black-1 my-3'>{item}</p>
    ))}
  </div>
}
const Footer = () => {
    ...
    <footer className="flexCenter flex-col border-t dark:border-nft-black-1 border-nft-gray-1 sm:py-8 py-16">
      <div className="w-full minmd:w-4/5 flex flex-row md:flex-col sm:px-4 px-16">
        ...
        <div className='flex-1 flexBetweenStart flex-wrap ml-10 md:ml-0 md:mt-8'>
          ...
          <FooterLinks heading="Cryptoket" items={["Help Center", "Terms of Service", "Legal", "Privacy Policy"]} />
        </div>
      </div>
      <div className="flexCenter w-full mt-5 border-t dark:border-nft-black-1 border-nft-gray-1 sm:px-4 px-16">
        <div className="flexBetween flex-row w-full min-w-4/5 sm:flex-col mt-7">
